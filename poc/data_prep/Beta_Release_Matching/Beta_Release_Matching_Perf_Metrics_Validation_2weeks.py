...
# MAGIC %md This is the same as `Beta_Release_Matching_Perf_Metrics_Validation`, with the time windows reduced by half to reflect the reduced launch timeframe from 7 to 4 weeks.
# MAGIC 
# MAGIC In addition, the mean of the engagement metrics was taken, rather than the sum. This enables the usage of a sliding windows of differing length for Beta and Release.
# MAGIC 
# MAGIC Finally, additional covariates were added for potential usage. 
...
DATA_PATH = 'cdowhygelund/beta_release_matching/2week_US/engage_means/Validation'
...
beta_ms = get_df(ms, 'beta', BETA_VALIDATION_START_DATE, 
...
beta_meta = get_meta(beta['df'])
beta_df = add_meta(beta['agg'], beta_meta)
beta_ms = get_df(ms, 'beta', BETA_TRAINING_START_DATE, 
