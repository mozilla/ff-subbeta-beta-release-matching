---
title: "Milestone 2: Hyperparameter Tuning - Nearest Neighbors, Logit w/Linear Propensity Score"
...
results_file_name <- 'hyperparameter_tuning_nn_logit_linear.RData' 
results_file_path <- file.path('data', results_file_name)
```

# Features
The optimal feature set was found in `feature_selection_nn_logit_linear.Rmd`. At the time of this run they were:
...
fs1_results <- perform_matchit_hp_tune(df_train_f, bts[['bts_4x']], fs1, size=NULL, workers = 64, hps_grid = hps_grid, 
                                       distance = 'linear.logit', add_interactions=TRUE)
...
```{r fs5, error=TRUE}
fs3_results <- perform_matchit_hp_tune(df_train_f, bts[['bts_8x']], fs5, size=NULL, workers = 64, hps_grid = hps_grid, 
                                       distance = 'linear.logit', add_interactions=FALSE)
save(list=ls(pattern = 'fs[0-9]_results'),  file = results_file_path)
...
knitr::kable(fs3_results_df[order(fs3_results_df$mean), ] %>% head(20))
```
