...
# MAGIC %md Lots of prior art from David Zeber (https://metrics.mozilla.com/~dzeber/notebooks/esper/esper-main-summary_model.html

# COMMAND ----------

# MAGIC %md # Methodology 1
...
NUM_DAYS = 14
NUM_WEEKS  = 4
BETA_START_DATE = dt.datetime(2019, 5, 21)
BETA_VERSION = '68'

RELEASE_START_DATE = dt.datetime(2019, 7, 9)
RELEASE_VERSION = BETA_VERSION

DATA_BUCKET = "net-mozaws-prod-us-west-2-pipeline-analysis"
DATA_PATH = 'cdowhygelund/beta_release_matching/4week/V2'
...
# MAGIC %md ## Beta Data Pull

# COMMAND ----------

sample_frac = 25

beta = get_df(cd, 'beta', BETA_START_DATE, (str(x) for x in range(1, 1+sample_frac)), BETA_VERSION, gen_metrics)
beta_meta = get_meta(beta['df'])
beta_df = add_meta(beta['agg'], beta_meta)
beta_ms = beta_ms = get_df(ms, 'beta', BETA_START_DATE, (str(x) for x in range(1, 1+sample_frac)), BETA_VERSION,
...
s3_path = "s3://{}/{}".format(DATA_BUCKET, DATA_PATH)
...
# MAGIC %md ## Release Data Pull

# COMMAND ----------

sample_frac = ('43', )

release = get_df(cd, 'release', RELEASE_START_DATE, sample_frac, RELEASE_VERSION, gen_metrics)
release_meta = get_meta(release['df'])
release_df = add_meta(release['agg'], release_meta)
release_ms = get_df(ms, 'release', RELEASE_START_DATE, sample_frac, RELEASE_VERSION, 
                    gen_hist_metrics, rename_cid=True, add_label=False)['agg'] 
...
with StringIO() as f:
  ...
  file_path = '{}/df_pd.csv'.format(DATA_PATH)
  s3_resource.Object(DATA_BUCKET, file_path).put(Body=f.getvalue())    

# COMMAND ----------

# MAGIC %md # Methodology 2

# COMMAND ----------

# MAGIC %md Same as 1 save,
# MAGIC 
# MAGIC * Utilize _last_ four weeks of Beta as this is more representative of Release

# COMMAND ----------

BETA_START_DATE = dt.datetime(2019, 6, 4)

DATA_PATH = 'cdowhygelund/beta_release_matching/4week/V3'
